...
static char *_allow_areas[] = {"USER", "DEVICE_ID", "HASH0_PUBLIC", "LOADER_EK", "DEVICE_EK", "AREA_LAST"};
static char *_allow_wl_areas[] = {
	"LOCK_HASH0_PUBLIC", "LOCK_LOADER_EK", "LOCK_DEVICE_EK", "LOCK_LAST",
	"SECUREBOOT", "OTHERS",
...
static int acm_mem_init(void)
{
	...
	setup_buf = memalign(32, 8);
	memset(setup_buf, 0, 8);
	handler = memalign(32, HANDLER_SIZE);
	set_config_break(0);
	set_acm_config(0);
	current_speed = CH9_USB_SPEED_UNKNOWN;
	mem_alloc_cnt = 0;
...
static void sendInReq(uint32_t length, uint8_t token, cvi_reqComplete complete, uint8_t *pRsp, uint8_t rspLen)
{
	uint16_t crc;
...
static void efusew_cmd(uint32_t length, uint8_t *ack_result, uint8_t ack_len)
{
	...
	if (strncmp((void *)((uintptr_t)cmdBuf + (uintptr_t)HEADER_SIZE), "efusew ",
				 strlen("efusew ")) == 0) {
		char cmd[128] = {'\0'};
		uint8_t area_idx = 0;
		uint8_t area_size = 0;
		...
		strncpy(cmd, (void *)((uintptr_t)cmdBuf + (uintptr_t)HEADER_SIZE),
			min((uint32_t)(length - HEADER_SIZE), (uint32_t)sizeof(cmd)));
		...
		for (area_idx = 0; area_idx < ARRAY_SIZE(_allow_wl_areas); area_idx++) {
			if (strncmp((void *)((uintptr_t)cmdBuf +
								(uintptr_t)HEADER_SIZE + strlen("efusew ")),
				_allow_wl_areas[area_idx], strlen(_allow_wl_areas[area_idx])) == 0) {
				area_idx += CVI_EFUSE_AREA_LAST + 1;

				NOTICE("area_idx = %d\n", area_idx);
				if (area_idx >= CVI_EFUSE_LOCK_WRITE_HASH0_PUBLIC) {
					efuse_locked = CVI_EFUSE_IsWriteLocked(area_idx);
					NOTICE("CVI_EFUSE_IsWriteLocked %d\n", efuse_locked);
				} else {
					efuse_locked = CVI_EFUSE_IsLocked(area_idx);
		...
		for (area_idx = 0; area_idx < ARRAY_SIZE(_allow_areas); area_idx++) {
			if (strncmp((void *)((uintptr_t)cmdBuf +
								(uintptr_t)HEADER_SIZE + strlen("efusew ")),
				_allow_areas[area_idx], strlen(_allow_areas[area_idx])) == 0) {
				...
				data_offset = HEADER_SIZE + strlen("efusew ") +
								strlen(_allow_areas[area_idx]) + 1;
				INFO("data_offset %x\n", data_offset);
				for (i = 0; i < (length - data_offset) / 2; i++) {
					uint8_t data;

					data = ahex2int(cmdBuf[data_offset + i * 2],
...
static void efuser_cmd(uint32_t length, uint8_t *read_buf, uint8_t buf_len)
{
	...
	if (strncmp((void *)((uintptr_t)cmdBuf + (uintptr_t)HEADER_SIZE), "efuser ",
				 strlen("efuser ")) == 0) {
		char cmd[128] = {'\0'};
		uint8_t area_idx = 0;
		uint8_t area_size = 0;
		...
		for (area_idx = 0; area_idx < ARRAY_SIZE(_allow_areas); area_idx++) {
			if (strncmp((void *)((uintptr_t)cmdBuf +
								(uintptr_t)HEADER_SIZE + strlen("efuser ")),
		...
		for (area_idx = 0; area_idx < ARRAY_SIZE(_allow_wl_areas); area_idx++) {
			if (strncmp((void *)((uintptr_t)cmdBuf + (uintptr_t)HEADER_SIZE + strlen("efuser ")),
				_allow_wl_areas[area_idx], strlen(_allow_wl_areas[area_idx])) == 0) {
				...
				memset(read_buf, 0, buf_len);
				if (area_idx >= CVI_EFUSE_LOCK_WRITE_HASH0_PUBLIC) {
					read_buf[0] = CVI_EFUSE_IsWriteLocked(area_idx);
					NOTICE("CVI_EFUSE_IsWriteLocked %d\n", read_buf[0]);
				} else {
					read_buf[0] = CVI_EFUSE_IsLocked(area_idx);
					NOTICE("CVI_EFUSE_IsLocked %d\n", read_buf[0]);
				}
				break;
			}
...
static void bulkOutCmplMain(struct usb_ep *ep, struct usb_request *req)
{
	...
	case CVI_USB_PRG_CMD:
		...
		for (i = 0; i < ARRAY_SIZE(_allow_cmds); i++) {
			if (0 == strncmp((void*)((uintptr_t)cmdBuf + (uintptr_t)HEADER_SIZE), _allow_cmds[i],
					 strlen(_allow_cmds[i]))) {
				char cmd[255] = {'\0'};
				strncpy(cmd, (void*)((uintptr_t)cmdBuf + (uintptr_t)HEADER_SIZE),
	...
	case CVI_USB_EFUSEW:
		...
		sendInReq(length, CVI_USB_EFUSEW, bulkResetOutReq, ack_result, sizeof(ack_result));
		break;

	case CVI_USB_EFUSER:
		NOTICE("CVI_USB_EFUSER\n");
		efuser_cmd(length, read_buf, sizeof(read_buf));
		sendInReq(length, CVI_USB_EFUSER, bulkResetOutReq, read_buf, 40);
		break;
	...
	case CVI_USB_READ_SN:
		...
		NOTICE("0x%x%x\n", sn_hi, sn_lo);
		sendInReq(length, CVI_USB_READ_SN, bulkResetOutReq, ack_result, sizeof(ack_result));
		break;
...
static int setup(struct usb_gadget *gadget,
	...
	*(status_value + 1) = 0;
	...
	case CH9_USB_REQ_TYPE_STANDARD:
		...
		case CH9_USB_REQ_GET_DESCRIPTOR:
			...
			if ((TmpCtrl.bmRequestType & CH9_REQ_RECIPIENT_MASK) == CH9_USB_REQ_RECIPIENT_INTERFACE) {
			...
			} else if ((TmpCtrl.bmRequestType & CH9_REQ_RECIPIENT_MASK) == CH9_USB_REQ_RECIPIENT_DEVICE) {
				switch (TmpCtrl.wValue >> 8) {
				case CH9_USB_DT_DEVICE:
					length = CH9_USB_DS_DEVICE;
					if (cvi_usb_vid != 0) {
						NOTICE("Patch VID %x\n", cvi_usb_vid);
						devDesc->idVendor = cpuToLe16(cvi_usb_vid);
					}
					if (cvi_usb_pid != 0) {
						NOTICE("Patch PID %x\n", cvi_usb_pid);
						devDesc->idProduct = cpuToLe16(cvi_usb_pid);
					}
				...
				case CH9_USB_DT_CONFIGURATION: {
					...
					uint16_t acmDescLen = (uint16_t)getDescAcm(gadget->speed, ptr);
...
{
	return (!!(mmio_read_32(REG_TOP_CONF_INFO) & BIT_TOP_CONF_INFO_VBUS));
}
#endif
