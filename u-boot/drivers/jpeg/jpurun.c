		...
		int interleave, int format, int endian, int packed)
{
	...
	CVI_JPG_DBG("strideY = %d, strideC = %d, lumaSize = 0x%X, chromaSize = 0x%X\n", strideY, strideC, lumaSize, chromaSize);
...
int jpeg_decode_helper(DecConfigParam *param)
{
	...
	ret = WriteJpgBsBufHelper(handle, &bufInfo, decOP.bitstreamBuffer, decOP.bitstreamBuffer + decOP.bitstreamBufferSize, 0, 0,
				  &streameos, decOP.streamEndian);
	...
	if (ret != JPG_RET_SUCCESS) {
		suc = 0;
		JLOG(ERR,
		     "JPU_DecGetInitialInfo failed Error code is 0x%x, inst=%d\n",
		     ret, instIdx);
		goto ERR_DEC_OPEN;
	}
	...
	if (decOP.packedFormat >= PACKED_FORMAT_422_YUYV &&
		decOP.packedFormat <= PACKED_FORMAT_422_VYUY) {
	...
	ret = JPU_DecRegisterFrameBuffer(handle, frameBuf,
			regFrameBufCount, framebufStrideY, framebufStrideC);
	if (ret != JPG_RET_SUCCESS) {
		suc = 0;
		...
		if (ret != JPG_RET_SUCCESS && ret != JPG_RET_EOS) {
			if (ret == JPG_RET_BIT_EMPTY) {
				ret = WriteJpgBsBufHelper(handle, &bufInfo, decOP.bitstreamBuffer,
							  decOP.bitstreamBuffer + decOP.bitstreamBufferSize, STREAM_FILL_SIZE, 0,
							  &streameos, decOP.streamEndian);
...
// frameBuf[partBufIdx].bufY);
			...
			if (int_reason & (1 << INT_JPU_DONE)) { // Must catch PIC_DONE
								// interrupt before catching
								// EMPTY interrupt
				// Do no clear INT_JPU_DONE and INT_JPU_ERROR
				// interrupt. these will be cleared in
				// JPU_DecGetOutputInfo.
				JLOG(INFO, "jpeg done\n");
				jpeg_done = 1;
				break;
			}

			if (int_reason & (1 << INT_JPU_ERROR)) { // Must catch PIC_DONE
								 // interrupt before catching
								 // EMPTY interrupt
				// Do no clear INT_JPU_DONE and INT_JPU_ERROR
				// interrupt. these will be cleared in
			...
			if (int_reason & (1 << INT_JPU_BIT_BUF_STOP)) {
				...
				if (ret != JPG_RET_SUCCESS) {
					suc = 0;
					JLOG(ERR,
					     "JPU_DecCompleteStop failed Error code is 0x%x\n",
					     ret);
					goto ERR_DEC_OPEN;
				}
...
JPU_END_OF_STREAM:
		...
		JLOG(INFO,
		     ...
		     instIdx, frameIdx, outputInfo.indexFrameDisplay, outputInfo.consumedByte, ppIdx, outputInfo.bytePosFrameStart,
		...
		if (dispImage) {
			...
			saveIdx = ppIdx;
			if (comparateYuv((Uint8 *)(frameBuf[saveIdx].bufY), pRefYuvBuf, dispWidth, dispHeight, framebufStrideY,
					 (int)decOP.chroma_interleave, framebufFormat, 0, decOP.packedFormat) != 0) {
				suc = 0;
			}
