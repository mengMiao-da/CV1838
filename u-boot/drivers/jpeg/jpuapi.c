...
JpgRet JPU_DecStartOneFrame(JpgDecHandle handle, JpgDecParam *param)
{
	...
	if (pDecInfo->errInfo.bError) {
		...
		JpuWriteReg(MJPEG_MCU_INFO_REG, pDecInfo->mcuBlockNum << 16 |
						    pDecInfo->compNum << 12 |
	...
	if (pDecInfo->packedFormat == PACKED_FORMAT_NONE)
		JpuWriteReg(
		    MJPEG_DPB_CONFIG_REG,
		    (pDecInfo->frameEndian << 6) | (0 << 5) | (0 << 4) |
			((pDecInfo->chroma_interleave == 0)
			     ? 0
			     : (pDecInfo->chroma_interleave == 1) ? 2 : 3));
	...
	if (rotMir & 0x10) {
		...
		for (i = 0; i < pDecInfo->bufNum; i++) {
			JpuWriteReg(
			    ...
			    pDecInfo->frameBufPool[(val * pDecInfo->bufNum) + i]
				.bufY);
			JpuWriteReg(
...
JpgRet JPU_EncStartOneFrame(JpgEncHandle handle, JpgEncParam *param)
{
	...
	JpuWriteReg(MJPEG_BBC_RD_PTR_REG, pEncInfo->streamBufStartAddr);
	JpuWriteReg(MJPEG_BBC_CUR_POS_REG, 0);
	JpuWriteReg(MJPEG_BBC_DATA_CNT_REG,
	...
	if (pEncInfo->packedFormat == PACKED_FORMAT_NONE)
		JpuWriteReg(
			...
			(0 << 2) |
			((pEncInfo->chroma_interleave == 0)
			     ? 0
			     : (pEncInfo->chroma_interleave == 1) ? 2 : 3));
	...
	JpuWriteReg(MJPEG_GBU_CTRL_REG,
			...
			<< 3); // stuffing "FF" data where frame end
	...
	val = (pEncInfo->format == FORMAT_420 ||
	       pEncInfo->format == FORMAT_422 || pEncInfo->format == FORMAT_400)
		  ? 2
		  : 1;

	if (pEncInfo->chroma_interleave)
		JpuWriteReg(MJPEG_DPB_CSTRIDE_REG,
			    (pBasFrame->stride / (int)val) * 2);
	else
		JpuWriteReg(MJPEG_DPB_CSTRIDE_REG,
			    pBasFrame->stride / (int)val);
...
JpgRet JPU_EncGetOutputInfo(JpgEncHandle handle, JpgEncOutputInfo *info)
{
	...
	info->bitstreamSize =
	    JpuReadReg(MJPEG_BBC_WR_PTR_REG) - pEncInfo->streamBufStartAddr;
	pEncInfo->streamWrPtr = JpuReadReg(MJPEG_BBC_WR_PTR_REG);
