...
int mtd_read_id(struct mtd_info *mtd)
{
	mtd->_read_id(mtd);
	return 0;
}
EXPORT_SYMBOL_GPL(mtd_read_id);
	     ...
	     u_char *buf)
{
	...
	*retlen = 0;
	 ...
	 */
	ret_code = mtd->_read(mtd, from, len, retlen, buf); //nand_read
...
int mtd_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	 ...
	 */
	ret_code = mtd->_read_oob(mtd, from, ops); //nand_read_oob
	if (unlikely(ret_code < 0))
		return ret_code;
	if (mtd->ecc_strength == 0)
		return 0;	/* device lacks ecc */
