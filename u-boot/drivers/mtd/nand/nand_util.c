...
int nand_verify(struct mtd_info *mtd, loff_t ofs, size_t len, u_char *buf)
{
	...
	// FIXME : temp disable block verify.
//	if (len >= mtd->erasesize && ((len & mtd->erasesize_mask) == 0) && ((ofs & mtd->erasesize_mask) == 0)) {
//		printf("nand verify block size aligned mtd->erasesize %d\n", mtd->erasesize);
//		verlen = mtd->erasesize;
//		block_cmp = 1;
//	}

	verbuf = memalign(mtd->writesize, verlen); // verlen align buffer
	...
	if (block_cmp) {
		for (verofs = ofs; verofs < ofs + len;
			...
			if (rval) {
				bbt_dump_buf("buf", buf, 0x20000);//verlen);
				bbt_dump_buf("verbuf", verbuf, 0x20000);//verlen);
				break;
			}
		}
	}
