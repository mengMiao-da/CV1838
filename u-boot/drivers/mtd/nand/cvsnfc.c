...
//		// for dma write
		...
		writel(0x26 << (ch * 8), 0x03000154); //PER dst => remap chx
		uint32_t val = readl(0x03000154) | 0x80000000; // update bit

		writel(val, 0x03000154); // set sdma remap update bit
		...
		writel(0x26 << (ch * 8), 0x03000154); //PER src => remap chx
		uint32_t val = readl(0x03000154) | 0x80000000; // update bit
		     ...
		     uint8_t *buf, int bytes, int page)
{
	...
	if (ret == -EBADMSG) {
		printf("%s : ECC status ECC_UNCORR on page 0x%x, block %d\n", __func__, page, blk_idx);
	}
...
freq_retry:
	...
	for (dly_line_sel = 0; dly_line_sel < 128; dly_line_sel++) {
		...
		if (!memcmp(&nand_id, host->nand_chip_info->id, host->nand_chip_info->id_len)) {
			...
			if (!memcmp(header_buf, pattern1, 4) && !memcmp(header_buf + 4, pattern2, 4)) {
	...
	if (dly_param_idx != 0) { /* at least find 1 parameter */
		...
		pr_info("Final selection, ctrl=0x%x, boot_prd=0x%x\n", cvsfc_read(host, REG_SPI_NAND_TRX_CTRL1),
			       ...
			       u32 data_offs, u32 readlen, u8 *buf, int page)
{
	...
	memcpy(buf, host->buffer + data_offs, readlen);
...
void cvsnfc_nand_init(struct nand_chip *chip)
{
	...
	chip->options     = NAND_NO_AUTOINCR | NAND_BROKEN_XD;// | NAND_BBT_SCANNED;
...
/*****************************************************************************/
extern void cvitek_spi_flash_pinmux(void);
...
{
	if (!snfc_host.chip) {
		pr_err("spi nand flash uninitialized.\n");
		return -1;
	}
