...
#define BLOCKALIGNED(x)	((x & ((1ULL << chip->phys_erase_shift) - 1)) == 0)  //(1ULL << chip->phys_erase_shift)
...
//	uint32_t max_oobsize = mtd_oobavail(mtd, ops);
	int block_size = 1ULL << chip->phys_erase_shift;
	int allow_block_read = 0;
	...
	if (BLOCKALIGNED(from) && readlen >= block_size && !col)
	{
		// FIXME : TEMP disable block read
//		allow_block_read = 1;
	}
	...
	while (1) {
		...
		aligned = (bytes & (mtd->writesize - 1)) ? 0 : 1;
		...
		if (realpage != chip->pagebuf || oob) {
			...
			if (use_bufpoi && aligned) // ??
				printf("%s: using read bounce buffer for buf@%p\n",
						 __func__, buf);
...
read_retry:
			...
			if (use_bufpoi) {
				...
				if (!aligned && NAND_HAS_SUBPAGE_READ(chip) && !oob)
					memcpy(buf, chip->buffers->databuf, bytes);
				else
		     ...
		     size_t *retlen, uint8_t *buf)
{
	...
	*retlen = ops.retlen;
	...
	pr_debug("from 0x%llx, len %ld, retlen %ld, buf %p\n", from, len, *retlen, buf);
...
static int nand_read_id(struct mtd_info *mtd)
{
	...
	printf("Get NAND id: ");
	if (host->nand_chip_info->id_len != 3)
		host->nand_chip_info->id_len = 2; /* set default id length to 2 */

	for (int i = 0;	i < host->nand_chip_info->id_len; i++) {
		nand_id[i] = chip->read_byte(mtd);
		printf("0x%x ", nand_id[i]);
	}
...
static int nand_read_oob_std(struct mtd_info *mtd, struct nand_chip *chip,
			     int page)
{
	chip->cmdfunc(mtd, NAND_CMD_READOOB, 0, page); // read_oob_data
	chip->read_buf(mtd, chip->oob_poi, mtd->oobsize);
	return 0;
}
			       ...
			       struct mtd_oob_ops *ops)
{
	...
	pr_debug("%s fr 0x%08Lx, l %i, mode %d\n", __func__, from, readlen, ops->mode);
	...
	while (1) {
		...
		if (ops->mode == MTD_OPS_RAW)
			...
			ret = chip->ecc.read_oob(mtd, chip, page); // cvsnfc_read_oob
 ...
 */
extern void bbt_dump_buf(void *buf, int len);
		...
		int oob_required, int page, int cached, int raw)
{
	...
	if (unlikely(raw))
		...
		status = chip->ecc.write_page(mtd, chip, buf, data_len, page); // cvsnfc_write_page
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	// FIXME: temp disable block write
	if (BLOCKALIGNED(to) && writelen >= block_size)
	{
//		allow_block_write = 1;
	}
	...
	while (1) {
		int bytes = allow_block_write ?  (block_size): mtd->writesize;
		...
		ret = chip->write_page(mtd, chip, column, bytes, wbuf, //nand_write_page
		...
		column = 0;
		buf += bytes;
		realpage += (bytes >> chip->page_shift);
			  ...
			  size_t *retlen, const uint8_t *buf)
{
	...
	pr_debug("nand write : to 0x%llx, len : %ld\n", to, len);
...
static int single_erase(struct mtd_info *mtd, int page)
{
	...
	return chip->waitfunc(mtd, chip); //cvsnfc_waitfunc
}
						  ...
						  struct nand_flash_dev *type)
{
	...
	*dev_id = chip->read_byte(mtd);
	...
	if (chip->options & NAND_BUSWIDTH_AUTO) {
		...
		pr_info("bus width %d instead %d bit\n",
...
extern int check_and_update_fip_bin(void);
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	if (!ecc->layout && (ecc->mode != NAND_ECC_SOFT_BCH)) {
		...
		case 128:
			...
			break;
		default:
			printf("No oob scheme defined for oobsize %d\n",
				   mtd->oobsize);
			BUG();
		}
	}
	...
	case NAND_ECC_HW_SYNDROME:
		...
		pr_warn("%d byte HW ECC not possible on %d byte page size, fallback to SW ECC %d\n",
			ecc->size, mtd->writesize, ecc->strength);
