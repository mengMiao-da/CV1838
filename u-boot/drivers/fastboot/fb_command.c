...
#endif
void oem_command(char *cmd_parameter, char *response);
...
int fastboot_handle_command(char *cmd_string, char *response)
{
	...
	printf("%s cmd_parameter %s,cmd_string  %s\n", __func__, cmd_parameter, cmd_string);
	strsep(&cmd_parameter, ":");

	for (i = 0; i < FASTBOOT_COMMAND_COUNT; i++) {
		printf("%s commands[i].command %s, %s\n", __func__, commands[i].command, cmd_string);
		...
		} else if (!strncmp(commands[i].command, cmd_string, 3)) { // for oem command
			if (commands[i].dispatch) {
				commands[i].dispatch(cmd_string, response);
				return i;
			}
		}
	}
...
void oem_command(char *cmd_parameter, char *response)
{
	...
	printf("%s cmd_parameter %s %s\n", __func__, cmd_parameter, response);

	removeSubstr(cmd_parameter, "oem ");

	printf("%s %s %s\n", __func__, cmd_parameter, response);

	sprintf(cmdbuf, cmd_parameter);
	if (run_command(cmdbuf, 0))
	{
		// save exec status for host to get the result
		oem_cmd_result = -1;
		fastboot_fail("", response);
	} else {
		oem_cmd_result = 0;
	...
	// Workaround : After tftp done, the connection will be broken
	// here we rerun the fastobot command
	if (!strncmp("tftp", cmd_parameter, 4)) {
		printf("tftp cmd!\n");
		sprintf(cmdbuf2, "fastboot udp");
		run_command(cmdbuf2, 0);
	}
