...
static void getvar_oem_cmd_result(char *var_parameter, char *response);
...
static const struct {
...
} getvar_dispatch[] = {
	{
		...
		.variable = "oem-cmd-result",
		.dispatch = getvar_oem_cmd_result
	}, {
...
extern int oem_cmd_result;

static void getvar_oem_cmd_result(char *var_parameter, char *response)
{
	printf("%s oem cmd resut %d\n", __func__, oem_cmd_result);
	if (oem_cmd_result)
		fastboot_okay("OEM CMD FAILED", response);
	else
		fastboot_okay("OEM CMD PASSED", response);
}
