...
// MIPI_TX reset
#if defined(CV1835_WDMB_0003A)
	_reg_write_mask(0x03022004, 0x300, 0x300);
	_reg_write_mask(0x03022000, 0x300, 0x200);

	_reg_write_mask(0x03021004, 0x10, 0x10);
	_reg_write_mask(0x03021000, 0x10, 0x10);

	// MIPI_TX reset
	_reg_write_mask(0x03022000, 0x300, 0x300);
	mdelay(10);
	_reg_write_mask(0x03022000, 0x300, 0x200);
	mdelay(10);
	_reg_write_mask(0x03022000, 0x300, 0x300);
	mdelay(100);
#else //EVB
	if (strstr(__stringify(CVICHIP), "cv1835")) {
		_reg_write(0x03021004, 0x38);
		mdelay(10);
		_reg_write(0x03021000, 0x18);
		mdelay(10);
		_reg_write(0x03021000, 0x38);
		mdelay(100);
	} else if (strstr(__stringify(CVICHIP), "cv1826")) {
		_reg_write(0x05021004, 0x07);
		_reg_write(0x05021000, 0x07);
		mdelay(10);
		_reg_write(0x05021000, 0x03);
		mdelay(10);
		_reg_write(0x05021000, 0x07);
		mdelay(100);
	} else {
