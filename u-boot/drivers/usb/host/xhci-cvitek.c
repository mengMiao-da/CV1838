...
#include "xhci.h"
#include "xhci-cvitek.h"

extern int onboard_hub_reset(void);

/* Declare global data pointer */

struct cvi_xhci_platdata {
	fdt_addr_t hcd_base;
};

static struct cvi_xhci cvitek;

static int cvitek_core_init(struct cvi_xhci *cvi)
{
	u32	reg;

	reg = xhci_readl(&cvi->regs->OTGVERSION);

	if ((OTGVERSION_TYPE__OTGVERSION__READ(reg)) != OTG_HW_VERSION) {
		printf("this is not a Cvitek USB3 OTG Core\n");
		return -ENODEV;
	}
	cvi->otg_version = OTGVERSION_TYPE__OTGVERSION__READ(reg);

	reg = xhci_readl(&cvi->regs->OTGSTS);
	if (OTGSTS_TYPE__OTG_NRDY__READ(reg) != 0) {
		printf("Cvitek USB3 OTG device not ready\n");
		return -ENODEV;
	}

	printf("%s otg_version:0x%x\n", __func__, cvi->otg_version);

	return 0;
}

void cvi_set_mode(struct cvi_xhci *cvi, u32 mode)
{
	u32 reg;

	switch (mode) {
	case CVI_ROLE_GADGET:
		xhci_writel(&cvi->regs->OTGCMD,
	...
	case CVI_ROLE_HOST:
		...
		break;
	case CVI_ROLE_OTG:
		reg = xhci_readl(&cvi->regs->OTGCTRL1);
		reg = OTGCTRL1_TYPE__IDPULLUP__SET(reg);
		xhci_writel(&cvi->regs->OTGCTRL1, reg);
		/*
	...
	if (cvi->ss_disable)
		xhci_writel(&cvi->regs->OTGCMD, xhci_readl(&cvi->regs->OTGCMD) |
				OTGCMD_TYPE__SS_HOST_DISABLED_SET__MASK);
	cvi->current_mode = mode;
}

static int cvitek_core_init_mode(struct cvi_xhci *cvi)
{
	...
	cvi_set_mode(cvi, CVI_ROLE_HOST);

	return 0;
}

static int cvitek_xhci_core_init(struct cvi_xhci *cvi,
				 struct udevice *dev)
{
	int ret = 0;

	ret = cvitek_core_init(cvi);
	if (ret) {
		debug("%s:failed to initialize core\n", __func__);
		return ret;
	}

	cvitek_core_init_mode(cvi);
	...
	xhci_writel((void *)(USB_HOST_BASE + 0x80d4),
		    ...
		    ~0x100));
	/*
	 * disable BW calculation for CV1835.
	 */
...
static void cvitek_xhci_core_exit(struct cvi_xhci *cdsn)
{
}

int xhci_hcd_init(int index, struct xhci_hccr **hccr, struct xhci_hcor **hcor)
{
	struct cvi_xhci *ctx = &cvitek;
	int ret = 0;

	ctx->hcd = (struct xhci_hccr *)USB_HOST_BASE;
	ctx->regs = (void *)USB_BASE;

	ret = cvitek_xhci_core_init(ctx, NULL);
	if (ret < 0) {
	...
	*hcor = (struct xhci_hcor *)((uintptr_t)*hccr
	...
	debug("cvitek-xhci: init hccr %lx and hcor %lx hc_length %ld\n",
...
{
	struct cvi_xhci_platdata *plat = dev_get_platdata(dev);
...
static int xhci_usb_probe(struct udevice *dev)
{
	...
	ctx->regs = (void *)USB_BASE;
	ret = cvitek_xhci_core_init(ctx, dev);
	if (ret) {
...
{
	struct cvi_xhci *ctx = dev_get_priv(dev);
	int ret;
...
U_BOOT_DRIVER(usb_xhci) = {
	...
	.platdata_auto_alloc_size = sizeof(struct cvi_xhci_platdata),
