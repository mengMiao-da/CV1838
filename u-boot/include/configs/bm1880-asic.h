...
#define CONFIG_EXTRA_ENV_SETTINGS	\
				...
				"othbootargs=earlycon debug user_debug=31 loglevel=10 " \
					"no_console_suspend crashkernel=512M-1G:80M,1G-:128M\0" \
				"uimage_addr=10F100000\0" \
				"checkupdatefile=fatload mmc 1:1 ${uimage_addr} sd_update_emmc.sh\0 " \
				"sdupdateboot= " \
					"echo sdupdateboot;" \
					"if run checkupdatefile ; then " \
...
#define CONFIG_BOOTCOMMAND	"run sdupdateboot || run bmrb || run bmtb || run bmsq || run bmdl || run ramboot || run sdboot || run emmcboot || run nfsboot"
