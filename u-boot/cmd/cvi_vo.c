...
static int dsi_init(int devno, const struct dsc_instr *cmds, int size)
{
	...
	for (int i = 0; i < size; i++) {
		...
		if (ret) {
			pr_debug("dsi init failed at %d instr.\n", i);
			return ret;
		}
	}
	return ret;
...
static int dsi_get_panel_id(int devno, unsigned int *id)
{
	...
	for (int i = 0; i < 3; i++) {
		...
		if (ret < 0) {
			pr_debug("dsi get panel id fail.\n");
			return ret;
		}
...
static int do_startvl(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (*argv[3] == 0 || *endp != 0)
		return CMD_RET_USAGE;
	stride = simple_strtoul(argv[4], &endp, 10);
	if (*argv[4] == 0 || *endp != 0)
		return CMD_RET_USAGE;
	rect.x = simple_strtoul(argv[5], &endp, 10);
	if (*argv[5] == 0 || *endp != 0)
		return CMD_RET_USAGE;
	rect.y = simple_strtoul(argv[6], &endp, 10);
	if (*argv[6] == 0 || *endp != 0)
		return CMD_RET_USAGE;
	rect.w = simple_strtoul(argv[7], &endp, 10);
	if (*argv[7] == 0 || *endp != 0)
		return CMD_RET_USAGE;
	rect.h = simple_strtoul(argv[8], &endp, 10);
	if (*argv[8] == 0 || *endp != 0)
		return CMD_RET_USAGE;
	...
	if (intf_type == SCLR_VO_INTF_MIPI) {
	...
	cfg->mem.addr1 = ALIGN(cfg->mem.addr0 + cfg->mem.pitch_y * ((rect.h + 1) & ~(BIT(0))), 0x1000);
	cfg->mem.addr2 = ALIGN(cfg->mem.addr1 + cfg->mem.pitch_c * ((rect.h + 1) >> 1), 0x1000);
...
U_BOOT_CMD(
	startvl,	9,	0,	do_startvl,
	"open video layer of the vo",
	"    - startvl [layer address_in address_out stride x y w h]"
);
