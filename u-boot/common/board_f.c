...
{
	struct pwm_dev pwm_14, pwm_15, pwm_0;

	pwm_14.pwm_id = 14;
	if (cv_pwm_config_mode(&pwm_14, 0, 0)) {
		printf("cv_pwm_config_mode fail\n");
		return -1;
	}
	if (cv_pwm_config_cycle(&pwm_14, 5000000, 10000000)) {
		printf("cv_pwm_config_cycle fail\n");
		return -1;
	}
	if (cv_pwm_enable(&pwm_14)) {
		printf("cv_pwm_enable fail\n");
		return -1;
	}
	if (cv_pwm_update(&pwm_14, 7000000, 10000000)) {
		printf("cv_pwm_update fail\n");
		return -1;
	}

	pwm_15.pwm_id = 15;
	if (cv_pwm_config_cycle(&pwm_15, 1000000, 10000000)) {
		printf("cv_pwm_config_cycle fail\n");
		return -1;
	}
	if (cv_pwm_enable(&pwm_15)) {
		printf("cv_pwm_enable fail\n");
		return -1;
	}
	...
	/* PWM0 sample code */
	pwm_0.pwm_id = 0;
	/* when duty = 1200/10000 = 12%, vout = 1.004v */
	cv_pwm_config_cycle(&pwm_0, 1200, 10000);
	cv_pwm_enable(&pwm_0);

	return 0;
}
#endif
#ifdef CONFIG_ADC_CVITEK
static int adc_operate(void)
{
	unsigned int adc_data;
	int channel;

	channel = 4;//Set the sample channel number 1, 2, 3, 4
	adc_data = cvitek_adc_read(channel);
	if (adc_data == -1)
		printf("invalid saradc channel%d\n", channel);
	else
		printf("adc channel%d value = %#x\n", channel, adc_data);

	channel = 1;//Set the sample channel number 1, 2, 3, 4
	adc_data = cvitek_adc_read(channel);
	if (adc_data == -1)
		printf("invalid saradc channel%d\n", channel);
	else
...
#endif /* end of CONFIG_ADC_CVITEK */
#endif
...
static const init_fnc_t init_sequence_f[] = {
	...
	show_board_info,
#endif
#if !defined(CONFIG_TARGET_CVITEK_CV1822)
